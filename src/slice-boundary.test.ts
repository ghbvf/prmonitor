...
//
// The vertical slices (config / pr / review / inbox) must be self-contained: a file in
...
const SLICES = ["config", "pr", "review", "inbox"] as const;
...
const sources = import.meta.glob("./{config,pr,review,inbox}/**/*.{ts,vue}", {
  query: "?raw",
  import: "default",
  eager: true,
...
describe("vertical slice boundary (config / pr / review / inbox)", () => {
