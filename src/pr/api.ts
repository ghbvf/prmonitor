...
export function pollNow(projectId: string): Promise<void> {
  return getTransport().request<void>("poll_now", { projectId });
}
...
export function startPolling(): Promise<void> {
  return getTransport().request<void>("start_polling");
}

export function stopPolling(): Promise<void> {
  return getTransport().request<void>("stop_polling");
}

export function reschedule(): Promise<void> {
  return getTransport().request<void>("reschedule");
}
...
export function onPrsUpdated(cb: (e: PrEvent) => void) {
  return getTransport().subscribe<PrEvent>(PRS_UPDATED_EVENT, cb);
}
...
export function setPrArchived(
  ...
  return getTransport().request<void>("set_pr_archived", { projectId, number, archived });
}

export function ghStatus(): Promise<GhStatus> {
  return getTransport().request<GhStatus>("gh_status");
}

export function azStatus(): Promise<AzStatus> {
  return getTransport().request<AzStatus>("az_status");
}
...
export function startWebhook(): Promise<WebhookStatus> {
  return getTransport().request<WebhookStatus>("start_webhook");
}

export function stopWebhook(): Promise<WebhookStatus> {
  return getTransport().request<WebhookStatus>("stop_webhook");
}

export function webhookStatus(): Promise<WebhookStatus> {
  return getTransport().request<WebhookStatus>("webhook_status");
}
...
export function webhookDeliveries(): Promise<WebhookDelivery[]> {
  return getTransport().request<WebhookDelivery[]>("webhook_deliveries");
}
...
export function pollStatus(projectId: string): Promise<PollStatus> {
  return getTransport().request<PollStatus>("poll_status", { projectId });
}
