...
// event subscription. Mirrors src/inbox/api.ts: backend access goes through the shared
// `../transport` port, and the wire types come from the `../types` root — the outbox
...
export function outboxList(projectId?: string): Promise<OutboxEntry[]> {
  return getTransport().request<OutboxEntry[]>("outbox_list", { projectId });
}
...
export function outboxGetRaw(id: number): Promise<string> {
  return getTransport().request<string>("outbox_get_raw", { id });
}
...
export function outboxRetry(id: number): Promise<void> {
  return getTransport().request<void>("outbox_retry", { id });
}
...
export function onOutboxUpdated(cb: (e: OutboxEvent) => void) {
  return getTransport().subscribe<OutboxEvent>(OUTBOX_UPDATED_EVENT, cb);
}
