...
|----|---------------------|------|--------|------------|
| 顶 | **Epic** | 能力工程聚合 | — | `epic` `backlog` `area` `pri` |
...
| Label | 领域 | 主要代码 |
|-------|------|--------|
| `area-config` | 配置、设置持久化与项目配置 | `src-tauri/src/config/` `src/config/` |
| `area-pr` | PR 来源、发现、标签与 source registry | `src-tauri/src/pr/` |
| `area-review` | review 引擎、session、评论 URL、deeplink/local API | `src-tauri/src/review/` |
| `area-dispatch` | 自动派发、scheduler、ledger、事件入口 | `src-tauri/src/dispatch.rs` `src-tauri/src/pr/scheduler.rs` `src-tauri/src/pr/ledger.rs` |
| `area-data` | SQLite 与持久化数据模型 | `src-tauri/src/db.rs` `src-tauri/src/*store*.rs` |
| `area-ui` | Vue UI、状态管理、类型契约消费 | `src/` |
| `area-tooling` | 构建、CI、Tauri 配置、治理脚本与技能 | `.github/` `.claude/` `.codex/` `hack/` `package.json` `src-tauri/Cargo.toml` |
| `area-cross` | 跨 ≥4 领域 / 无明确归属 | `src-tauri/src/model.rs` `src-tauri/src/error.rs` `src-tauri/src/events.rs` |
...
`cx-1` / `cx-2` / `cx-3` / `cx-4`（语义见 §3.2 rubric）。与 pri 同为评级两轴之一、载体对称（都是 label），cx **必填**：建 issue 时必须定级并显式 `--label cx-X`（与 pri 对称，无 unknown sentinel——定不到级也要在 §3.2 rubric 里就近取一档）；review/fix finding 派生的 issue 从 finding 的 `[…Cx…]` tag 自动带上对应 cx。epic / feature 容器不贴（跨多 PR、无单一 diff，§1.1）。PBI 叶子的 area/type/pri/cx 完整性由 `hack/automation/issue-labels.sh validate` 守卫（`issues` B1 建单前强制门；selftest 直接运行：`bash hack/automation/issue-labels.sh selftest`）。
...
*统一/合并/拆分/抽象/converge/unify/dedup/single source/funnel/sealed/Hard 升级* ；或触及跨切片契约 `src-tauri/src/model.rs` /
事件 union `src-tauri/src/events.rs` / 错误漏斗 `src-tauri/src/error.rs` / trait seam `PrSource` 或 `ReviewEngine`；或 AI-robust Soft→Hard / Funnel 双向锁未闭合；或影响 ≥3 领域。
...
| **Cx2** | 同领域 ≤5 文件 | 可能需 clippy / vitest / serde golden 单条 | 加方法、抽 helper、补契约测试 |
| **Cx3** | 跨领域 5–15 文件 | 需 trait seam / 类型系统 / 前后端契约同步 | trait 扩字段 + 多实现同步、`model.rs`↔`types.ts` 同步 |
| **Cx4** | ≥15 文件 / ≥3 领域 | 跨切片契约或生命周期状态机重构 | 派发状态机、进程生命周期、跨 UI/Rust wire 契约重构 |
...
离线契约测试直接运行：`bash hack/automation/pr-meta.sh selftest`（离线，无网络）。
...
/ship <issue>
  ...
  → 切 label 后 CI 异步收敛（非阻塞；capability-gated：激活 forge=azure 无 CI，ci-* 返回 no-ci，降级本地 pnpm/cargo 验证，不贴 pm:ci）
...
#   按领域：backlog + area-review        按类型：backlog + type-bug
