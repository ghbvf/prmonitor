...
# 每次 fix 首次发该信号 → deny 并回喂自检问题，提示模型按三原则复审本次 fix 方案后重发即放行。
...
#   .claude/settings.json（团队共享，本仓刻意选择）。
# 两层过滤：settings.json 的 hook `if: Bash(bash *fix-self-audit.sh*)` 粗筛——只在 bash 调用本脚本时
#   spawn（cargo test/pnpm/ls/cat/chmod 等不触发、无误导 statusMessage）；下面的 case 精筛 emit 实参。
# AI-robust：与 exitplan-self-audit.sh 同类的「自检提醒」hook，本质 Soft（按命令名锚定 + deny 回喂），
#   非约束 enforcement（不 enforce 不变式），不落 `.claude/rules/prmonitor/ai-robust.md`「Soft 严禁立项」范围；
#   属对既有先例的复制。
set -euo pipefail
...
jq -nc '{
  hookSpecificOutput: {
    ...
    permissionDecisionReason: "本 fix 方案符合 彻底 / 不向后兼容 / 优雅简洁 三原则吗？（见 fix/SKILL.md 阶段 3 方案设计原则）"
  }
}'
touch "$state"
