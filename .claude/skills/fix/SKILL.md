...
从问题代码向上（调用方）和向下（被调用方）追踪。跨 3+ 模块/切片用 Agent(Explore)。
同时追踪数据流：数据源 → 变换 → 消费者。
...
2. 是否需要修改 trait seam（`PrSource` / `ReviewEngine`）或跨切片契约（`model.rs` ↔ `types.ts`）？
3. 是否需要修改 SQLite schema / migration / 持久化数据形状？
4. 是否影响组装根 `src-tauri/src/lib.rs` 或 Tauri command 暴露面？
...
1. **Rust 标准库 / Tauri / Vue 官方文档或源码** → 有做法必须遵循，不自创。
2. **组件官方库**（tokio / serde / rusqlite / reqwest / Pinia 等）→ 遵循推荐模式 + 检查 Issues 已知陷阱。
3. **对标项目/社区惯例** → 参考，可偏离但须注明理由。
...
**Q2: 能不能现在做？** 检查：已有 issue 依赖、活跃分支冲突、trait seam 消费方、前后端契约同步成本。
...
| 复杂度                                                      | 条件 | 决策 |
|----------------------------------------------------------|------|-----|
| Cx1/Cx2 + IN_SCOPE + 不改 trait seam / migration / 组装根 / 并发或进程生命周期语义 | 全满足 | 直接修 |
...
- 最后两项固定：`commit + push` + `闭合/创建 backlog issue`
- 创建后立即执行，不等确认
...
scope 按切片/领域：`config` / `pr` / `review` / `dispatch` / `data` / `ui` / `tooling`。安全约束：只 add 修复文件（不 add -A）；不 amend。

### 4.2 执行代码修改（逐编辑测试循环）

> **批量并行**：4+ 条 finding 时按切片/领域（`config` / `pr` / `review` / `dispatch` / `data` / `ui` / `tooling`）聚类派发 `developer` sub-agent（**同切片同 agent** 防写冲突，组内串行执行下面循环）；并发 4-9→2 / ≥10→3；≤3 条由主 agent 直接处理。triage 同理可按聚类并行（`Explore`）。
...
2. Read 目标文件
3. Edit / Write 修改代码
4. Rust 改动运行 `cargo build --manifest-path src-tauri/Cargo.toml --locked`；前端改动运行 `pnpm build`
5. Rust 改动运行 `cargo test --manifest-path src-tauri/Cargo.toml --locked <test-name>`；前端改动运行 `pnpm test <test-name>`（含阶段 1.4 的复现测试）
6. 如果测试失败：
   - 分析失败原因
