...
必读：CLAUDE.md + `.claude/rules/prmonitor/*.md`（repo 级规则，develop/PR 合入后可用；若任一文件缺失则 fail-fast，不执行后续审查流程）+
...
- 必读：阶段 3 的共享上下文和全部 `.claude/rules/prmonitor/*.md`。
...
4. **修复分流** — Cx1/Cx2 簇 → `/fix`；Cx3/Cx4 簇 → "需人工决策" + 三级方案种子（最小/彻底/重构）。若 PR body（阶段 3 已取）含 forge closing/ref keyword（`close[sd]?` / `fix(e[sd])?` / `resolve[sd]?` / `refs`，大小写不敏感）后跟 `#<N>`，分流条目附 `← issue #<N>`
