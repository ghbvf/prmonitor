#!/usr/bin/env bash
# pr-meta.sh — produce and consume the prmonitor-pr-meta:v1 machine block that
...
#   <!-- prmonitor-pr-meta:v1 <standard-base64(JSON)> -->
#
...
# repo-slug). Path overridable via PRMONITOR_FORGE_SH for offline selftests.
FORGE_SH="${PRMONITOR_FORGE_SH:-${REPO_ROOT}/hack/automation/forge.sh}"
forge() { bash "${FORGE_SH}" "$@"; }
...
PRCOMMENTS_SH="${PRMONITOR_PRCOMMENTS_SH:-${REPO_ROOT}/hack/automation/pr-comments.sh}"

usage() {
    cat >&2 <<'EOF'
usage: pr-meta.sh <emit-block|decode|extract|round|selftest> [args]
  emit-block --kind=K --pr=N [flags]   build + print the prmonitor-pr-meta:v1 block
...
SCHEMA_CONST = "prmonitor-pr-meta/v1"
MARKER = "prmonitor-pr-meta:v1"
BLOCK_RE = re.compile(r"<!--\s*prmonitor-pr-meta:v1\s+([A-Za-z0-9+/=]+)\s*-->")
...
def do_decode(schema):
    ...
    if not blocks:
        sys.stderr.write("pr-meta decode: no valid prmonitor-pr-meta:v1 block found\n")
        sys.exit(2)
...
def do_selftest(schema):
    ...
    try:
        ...
        obj["idempotencyKey"] = "gocell/prmonitor#42@notahexsha:ship/ship#1"
        payload = base64.b64encode(canon(obj).encode("utf-8")).decode("ascii")
        block_line = "<!-- %s %s -->" % (MARKER, payload)
        assert_decode_fails(name, block_line)
    except Exception as e:
    ...
    def _minimal(kind, round_base, **extra):
        m = {"repo": "gocell/prmonitor", "pr": 42, "kind": kind, "tool": "claude-code",
